...
func (bs *BlockStore) LoadBlock(height int64) []byte {
	...
	buf := []byte{}
	for i := 0; i < blockMeta.PartSetSize; i++ {
...
func (bs *BlockStore) LoadBlockPart(height int64, partIndex int) []byte {
	bz, err := bs.db.Get(calcBlockPartKey(height, partIndex))
	if err != nil {
		panic(err)
	}
	if len(bz) == 0 {
...
func (bs *BlockStore) PruneBlocks(height int64) (uint64, error) {
	...
	for h := base; h < height; h++ {
		...
		for partIndex := 0; partIndex < blockMeta.PartSetSize; partIndex++ {
			if err := batch.Delete(calcBlockPartKey(h, partIndex)); err != nil {
				return 0, err
			}
		}
		pruned++
...
func (bs *BlockStore) saveBlockPart(height int64, partIndex int, part []byte, batch dbm.Batch, saveBlockPartsToBatch bool) {
	var err error
	if saveBlockPartsToBatch {
		err = batch.Set(calcBlockPartKey(height, partIndex), part)
	} else {
		err = bs.db.Set(calcBlockPartKey(height, partIndex), part)
	}
	if err != nil {
		panic(err)
	}
...
func (bs *BlockStore) saveStateAndWriteDB(batch dbm.Batch, errMsg string) error {
	...
	err := batch.WriteSync()
	if err != nil {
		return fmt.Errorf("error writing batch to DB %q: (height %d): %w",
			errMsg, bs.height, err)
	}
	return nil
}
